...
class LandinpageController {
  ...
  async index ({ request, response, params }) {
    try {
      ...
      landinpage.push(category)
      const contact = await Contact.findBy('user_id', user.id)
      landinpage.push(contact)
      const footer = await Footer.findBy('user_id', user.id)
      landinpage.push(footer)
      const galery = await Galery.query().where('user_id', user.id).with('images').first()
      const presentetions = await Presentetion.query().where('user_id',user.id).fetch()
      landinpage.push(presentetions)
      const profile = await Profile.findBy('user_id',user.id)
      landinpage.push(profile)
      const social = await Social.query().where('user_id', user.id).fetch()
      landinpage.push(social)
      const testimonial = await Testimonial.query().where('user_id', user.id).fetch()
      landinpage.push(testimonial)
